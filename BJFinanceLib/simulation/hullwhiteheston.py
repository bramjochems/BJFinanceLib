...
class HullWhiteHestonGenerator():
    ...
    def __init__(self, eq_spot, eq_vol_initial,
                 ...
                 rho_x_y, rho_S_sigma, rho_S_x, rho_S_y, rho_sigma_x,
                 rho_sigma_y, rates, sample_times):
        """
        Constructor. Doc to do.
        """
        self.sample_times = preprocessSampleTimes(sample_times)
        self._dt = [t-s for (s, t) in
                    list(zip(self.sample_times[:-1], self.sample_times[1:]))]
        ...
        self._initialize_cached_values(eq_spot, eq_vol_initial,
                                       ...
                                       eq_vol_of_vol, hw_a, hw_sigma_short_rate,
                                       hw_b, hw_sigma_long_rate, rho_x_y,
                                       rho_S_sigma, rho_S_x, rho_S_y,
                                       rho_sigma_x, rho_sigma_y)
    ...
    def _initialize_cached_values(self, eq_spot, eq_vol_initial,
                                  ...
                                  eq_vol_of_vol, hw_a, hw_sigma_short_rate,
                                  hw_b, hw_sigma_long_rate, rho_x_y,
                                  rho_S_sigma, rho_S_x, rho_S_y, rho_sigma_x,
                                  rho_sigma_y):
        """
    ...
    def _initialize_sigma_params(self):
        ...
        self._precomputed['sigma_params']['s2_constant'] = \
            [smean*omega**2*(1-edt)**2/(2*epsilon) for edt in exp_edt]
        self._precomputed['sigma_params']['s2_factor'] = \
            [omega**2*edt*(1-edt)/epsilon for edt in exp_edt]
        self._precomputed['sigma_params']['m_constant'] = \
            [smean*(1-edt) for edt in exp_edt]
        self._precomputed['sigma_params']['m_factor'] = \
            [edt for edt in exp_edt]
    ...
    def _initialize_xy_params(self):
        ...
        for name in ['x', 'y']:
            ...
            self._precomputed[name+'path']['mean_rev'] = \
                [exp(-self._inputs['hullwhite_' + name + '_meanreversion']*dt) for dt in self._dt]
            self._precomputed[name+'path']['stdev'] = \
                [sqrt((self._inputs['hullwhite_ ' + name + '_vol']**2)*
